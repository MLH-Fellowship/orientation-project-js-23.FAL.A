...
function PersonalInfoForm() {
    ...
    const formatPhoneNumber = (phoneNumber) => {
        ...
        numericOnlyNumber = numericOnlyNumber.substring(0,Math.min(PHONE_NUMBER_LENGTH,numericOnlyNumber.length)); // If the number is too long, will trim it
        return numericOnlyNumber;
    };

    const formatPhonePrefix = (phonePrefix) => {
        let formattedPrefix = phonePrefix.substring(0,Math.min(MAX_PHONE_PREFIX_LENGTH,phonePrefix.length));    // Trims if the phone prefix is too long
    ...
    return <div className="personal-form-container">
        <form className="personal-form">
            <div className="form-item">
                <input type="text" value={values.name} placeholder={placeholders.name} size={DEFAULT_INPUT_SIZE} onChange={e => handleSetValueChanges('name',e)}/>
                <div className="error-message">{errors.name}</div>
            </div>
            <div className="form-item">
                <div className="phone-form-item">
                    <input type="text" className="prefix-input" name="phonePrefix" id="phonePrefix" placeholder="+" value={values.phonePrefix} size={MAX_PHONE_PREFIX_LENGTH} onChange={e => handleSetValueChanges('phonePrefix', e)}/>
                    <input type="tel" className="phone-input" name="phone" id="phone" value={values.phone} placeholder={placeholders.phone} size={DEFAULT_INPUT_SIZE - 1} onChange={e => handleSetValueChanges('phone', e)}/>
                </div>
                <div className="error-message">{errors.phone ? errors.phone : errors.phonePrefix}</div>
            </div>
            <div className="form-item">
                <input type="email" name="email" id="email" value={values.email} placeholder={placeholders.email} size={DEFAULT_INPUT_SIZE} onChange={e => handleSetValueChanges('email',e)}/>
                <div className="error-message">{errors.email}</div>
            </div>
        </form>
        <button onClick={handlePersonalInfoSubmit} className="update-button">Update</button>
    </div>
}
